...
//
// Shared NAX class hierarchy used by both the gemm and attn trees.
// This is the canonical definition of Role, BaseNAXFrag, NAXFrag32,
// NAXTile, and tile_matmad_nax.  Tree-specific headers (gemm/nax.h and
// attn/nax.h) re-export from here; all new NAX code should go here.
...
struct NAXTile {
  ...
  // Convenience overloads for BaseNAXFrag (kPacking==2) callers that do not
  // need role/transpose or scratch. These match the old attn/nax.h NAXTile API
  // and are only valid when NAXFrag_t::kPacking == 2.  Calling them with a
  // kPacking==1 frag is a compile-time error (static_assert).

  template <typename U>
  METAL_FUNC void load(const device U* src, const int ld) {
    static_assert(
        NAXFrag_t::kPacking == 2,
        "NAXTile::load(src, ld) is only valid for kPacking==2 (BaseNAXFrag); "
        "use load<role, transpose>(src, ld, scratch) for NAXFrag32");
    const_for_loop<0, kTileRows, 1>([&](auto idx_row) {
      const_for_loop<0, kTileCols, 1>([&](auto idx_col) {
        NAXFrag_t::load(
            frag_at<idx_row.value, idx_col.value>(),
  ...
  template <typename U>
  METAL_FUNC void load_rows(const device U* src, const int ld,
                            const short n_rows) {
    static_assert(
        NAXFrag_t::kPacking == 2,
        "NAXTile::load_rows(src, ld, n_rows) is only valid for kPacking==2 "
        "(BaseNAXFrag); use load_rows<role, transpose>(src, ld, n_rows, "
        "scratch) for NAXFrag32");
    const_for_loop<0, kTileRows, 1>([&](auto idx_row) {
      const_for_loop<0, kTileCols, 1>([&](auto idx_col) {
        NAXFrag_t::load_rows(
            frag_at<idx_row.value, idx_col.value>(),
  ...
  METAL_FUNC void store(device U* dst, const int ld) const {
    static_assert(
        NAXFrag_t::kPacking == 2,
        "NAXTile::store(dst, ld) is only valid for kPacking==2 (BaseNAXFrag); "
        "use store(dst, ld, scratch) for NAXFrag32");
    const_for_loop<0, kTileRows, 1>([&](auto idx_row) {
      const_for_loop<0, kTileCols, 1>([&](auto idx_col) {
        NAXFrag_t::store(
            frag_at<idx_row.value, idx_col.value>(),
